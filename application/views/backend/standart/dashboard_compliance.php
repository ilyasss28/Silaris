...
$status_description = $is_submitted
  ? 'Notaris yang telah melengkapi laporan untuk seluruh bulan wajib pada periode terpilih.'
  : 'Notaris yang masih memiliki satu atau lebih bulan wajib yang belum dilaporkan.';
...
.cp-missing-months{min-width:190px;color:#b33a48!important;line-height:1.5}
@media(max-width:760px){.cp-table{min-width:900px}}
</style>
    ...
    <?php if ($compliance_rows): ?>
      <div class="cp-table-wrap">
        <table class="cp-table">
          <thead><tr><th>No.</th><th>Nama Notaris</th><th>Nomor Telepon</th><th>Wilayah</th><th>Status</th><th>Kelengkapan Bulan</th><th>Bulan Belum Dilaporkan</th><th>Laporan Terakhir</th></tr></thead>
          <tbody>
            <?php foreach ($compliance_rows as $index => $notary): ?>
              <?php $phone_display = $notary['phone_number'] === '-' ? '-' : format_phone_number($notary['phone_number']); ?>
              <tr data-compliance-search-row data-search="<?= _ent(strtolower($notary['display_name'] . ' ' . $phone_display . ' ' . $notary['region_name'] . ' ' . $notary['missing_month_labels'])); ?>">
                <td><?= $index + 1; ?></td>
                ...
                <td><?= (int) $notary['reported_month_count']; ?>/<?= (int) $notary['required_month_count']; ?> bulan</td>
                <td class="cp-missing-months"><?= _ent($notary['missing_month_labels']); ?></td>

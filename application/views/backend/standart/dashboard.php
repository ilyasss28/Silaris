...
$user_compliance = isset($dashboard_user_compliance) ? $dashboard_user_compliance : null;
$user_compliance_complete = $user_compliance && $user_compliance['status'] === 'submitted';
$user_required_months = $user_compliance ? (int) $user_compliance['required_month_count'] : (int) date('n');
$user_reported_months = $user_compliance ? (int) $user_compliance['reported_month_count'] : 0;
$user_progress = $user_required_months > 0
  ? min(100, (int) round(($user_reported_months / $user_required_months) * 100))
  : 0;
$user_compliance_hue = (int) round(($user_progress / 100) * 120);
$profile_copy = [
  'executive' => [
...
.dash-monitoring-grid>.dash-panel{height:430px;display:flex;flex-direction:column;overflow:hidden}.dash-monitoring-grid>.dash-panel:only-child{width:100%;grid-column:1/-1}.dash-monitoring-grid>.dash-panel>.dash-panel__head{flex:0 0 auto}.dash-scroll-area{min-height:0;flex:1;overflow-y:auto;overscroll-behavior:contain;scrollbar-width:thin;scrollbar-color:#c6ccd6 transparent}.dash-scroll-area::-webkit-scrollbar{width:6px}.dash-scroll-area::-webkit-scrollbar-track{background:transparent}.dash-scroll-area::-webkit-scrollbar-thumb{border-radius:99px;background:#c6ccd6}.dash-scroll-area::-webkit-scrollbar-thumb:hover{background:#aeb6c3}.dash-scroll-area.region-list{padding-right:8px}
.empty-state{min-height:170px;display:flex;flex-direction:column;align-items:center;justify-content:center;color:#8b95a4;text-align:center}.empty-state i{margin-bottom:10px;color:#c5cbd5;font-size:28px}.empty-state strong{color:#596579;font-size:11px}.empty-state span{margin-top:4px;font-size:9.5px}
...
.user-compliance{--compliance-tone:hsl(0 72% 40%);margin-bottom:18px;padding:20px;display:grid;grid-template-columns:auto minmax(0,1fr) auto;align-items:center;gap:18px;border:1px solid color-mix(in srgb,var(--compliance-tone) 28%,#fff);border-left:4px solid var(--compliance-tone);border-radius:11px;background:linear-gradient(90deg,color-mix(in srgb,var(--compliance-tone) 6%,#fff) 0,#fff 52%);box-shadow:0 3px 10px rgba(15,23,42,.035);transition:border-color .25s,background .25s}.user-compliance__icon{width:48px;height:48px;display:grid;place-items:center;border-radius:11px;background:color-mix(in srgb,var(--compliance-tone) 13%,#fff);color:var(--compliance-tone);font-size:19px}.user-compliance__eyebrow{margin-bottom:4px;color:var(--compliance-tone);font-size:9px;font-weight:800;letter-spacing:.055em;text-transform:uppercase}.user-compliance__title{margin:0;color:var(--dash-ink);font-size:14px;font-weight:800}.user-compliance__description{margin:5px 0 0;color:#687386;font-size:10.5px;line-height:1.55}.user-compliance__months{margin-top:8px;display:flex;align-items:flex-start;gap:6px;color:var(--compliance-tone);font-size:9.5px;line-height:1.5}.user-compliance__months i{margin-top:2px}.user-compliance__progress{min-width:190px}.user-compliance__progress-meta{margin-bottom:7px;display:flex;align-items:center;justify-content:space-between;gap:12px;color:#697487;font-size:9px}.user-compliance__progress-meta strong{color:var(--dash-ink);font-size:10.5px}.user-compliance__track{height:8px;overflow:hidden;border-radius:999px;background:color-mix(in srgb,var(--compliance-tone) 14%,#fff)}.user-compliance__fill{height:100%;border-radius:999px;background:var(--compliance-tone);transition:width .3s,background .25s}.user-compliance__actions{margin-top:11px;display:flex;justify-content:flex-end;gap:7px}.user-compliance__button{height:34px;padding:0 12px;display:inline-flex;align-items:center;justify-content:center;gap:6px;border:1px solid #d7dee8;border-radius:8px;background:#fff;color:#4c586c;font-size:9px;font-weight:800;text-decoration:none;white-space:nowrap}.user-compliance__button:hover{border-color:#aeb8c7;color:var(--dash-navy)}.user-compliance__button--primary{border-color:var(--dash-navy);background:var(--dash-navy);color:#fff}.user-compliance__button--primary:hover{background:#111064;color:#fff}
...
@media(max-width:1100px){.dash-stat-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.quick-access-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.dash-chart-filter__fields{grid-template-columns:repeat(3,minmax(120px,1fr))}.dash-filter-actions{grid-column:1/-1;justify-content:flex-end}}@media(max-width:860px){.dash-grid,.dash-grid--equal,.dash-grid--single{grid-template-columns:1fr}.dash-hero__mark{display:none}.user-compliance{grid-template-columns:auto minmax(0,1fr)}.user-compliance__progress{grid-column:1/-1}}@media(max-width:575px){.role-dashboard{padding-bottom:0}.dash-hero{min-height:0;padding:23px 20px}.dash-hero h1{font-size:20px}.dash-stat-grid{grid-template-columns:1fr}.dash-stat{min-height:116px}.dash-chart-filter{padding:16px}.dash-chart-filter__fields{grid-template-columns:1fr}.dash-filter-actions{grid-column:auto}.dash-filter-submit,.dash-filter-reset{flex:1}.trend-chart{gap:6px}.trend-chart__plot{gap:5px}.quick-access-grid{grid-template-columns:1fr}.attention-table th:nth-child(2),.attention-table td:nth-child(2){display:none}.user-compliance{padding:17px;grid-template-columns:1fr}.user-compliance__icon{width:42px;height:42px}.user-compliance__progress{grid-column:auto;min-width:0}.user-compliance__actions{justify-content:stretch;flex-direction:column}.user-compliance__button{width:100%}}
</style>
...
<section class="content role-dashboard">
  ...
  <?php if ($dashboard_profile === 'user'): ?>
    <article class="user-compliance" style="--compliance-tone:hsl(<?= $user_compliance_hue; ?> 72% 40%)" aria-label="Pengingat kepatuhan laporan bulanan">
      <div class="user-compliance__icon"><i class="fa <?= $user_compliance_complete ? 'fa-check-circle' : 'fa-bell'; ?>" aria-hidden="true"></i></div>
      <div class="user-compliance__content">
        <div class="user-compliance__eyebrow">Kepatuhan Laporan <?= (int) date('Y'); ?></div>
        <h2 class="user-compliance__title"><?= $user_compliance_complete ? 'Laporan bulanan Anda sudah lengkap' : 'Segera lengkapi laporan bulanan Anda'; ?></h2>
        <p class="user-compliance__description">
          <?= $user_compliance_complete
            ? 'Seluruh laporan wajib dari Januari sampai bulan berjalan telah tercatat di SILARIS.'
            : 'Status kepatuhan dinyatakan lengkap setelah seluruh bulan wajib sampai bulan berjalan dilaporkan.'; ?>
        </p>
        <?php if (!$user_compliance_complete): ?>
          <div class="user-compliance__months">
            <i class="fa fa-exclamation-circle" aria-hidden="true"></i>
            <span><strong>Bulan yang belum dilaporkan:</strong> <?= _ent($user_compliance ? $user_compliance['missing_month_labels'] : 'Data profil notaris belum terhubung'); ?></span>
          </div>
        <?php endif; ?>
      </div>
      <div class="user-compliance__progress">
        <div class="user-compliance__progress-meta"><span>Kelengkapan bulan</span><strong><?= $user_reported_months; ?>/<?= $user_required_months; ?> bulan</strong></div>
        <div class="user-compliance__track" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="<?= $user_progress; ?>"><div class="user-compliance__fill" style="width:<?= $user_progress; ?>%"></div></div>
        <div class="user-compliance__actions">
          <a class="user-compliance__button" href="<?= site_url('laporan'); ?>"><i class="fa fa-list" aria-hidden="true"></i> Lihat Laporan</a>
          <?php if (!$user_compliance_complete): ?>
            <a class="user-compliance__button user-compliance__button--primary" href="<?= site_url('laporan/add'); ?>"><i class="fa fa-upload" aria-hidden="true"></i> Unggah Laporan</a>
          <?php endif; ?>
        </div>
      </div>
    </article>
  ...
  <div class="dash-grid <?= $dashboard_profile === 'user' ? 'dash-grid--equal' : ''; ?>">
    <article class="dash-panel">
      <div class="dash-panel__head">
        <div><h2 class="dash-panel__title">Akumulasi Laporan Bulanan</h2><p class="dash-panel__subtitle">Jumlah notaris unik yang mengirim laporan bulanan pada periode terpilih.</p></div>
        <span class="dash-panel__tag"><?= _ent($dashboard_period); ?> · <?= number_format($chart_total, 0, ',', '.'); ?> notaris melapor</span>
      </div>
      <div class="trend-chart" role="img" aria-label="Grafik akumulasi laporan bulanan periode <?= _ent($dashboard_period); ?>">
        ...
        <div class="trend-chart__viewport">
          <div class="trend-chart__plot" style="--trend-columns:<?= count($dashboard_trend); ?>;min-width:<?= count($dashboard_trend) > 14 ? count($dashboard_trend) * 38 : 0; ?>px">
            <?php foreach ($dashboard_trend as $trend): ?>
              ...
              <div class="trend-bar" title="<?= _ent($trend['label']); ?>: <?= number_format($trend['total'], 0, ',', '.'); ?> notaris melapor">
    ...
    <article class="dash-panel">
      <div class="dash-panel__head">
        <div><h2 class="dash-panel__title"><?= $dashboard_profile === 'user' ? 'Status Pelaporan Saya' : 'Kepatuhan Pelaporan'; ?></h2><p class="dash-panel__subtitle">Patuh jika lengkap pada setiap bulan wajib periode <?= _ent($dashboard_period); ?>.</p></div>
  ...
  <?php if ($dashboard_profile === 'executive'): ?>
    <div class="dash-grid dash-grid--equal dash-monitoring-grid">
      ...
      <article class="dash-panel">
        <div class="dash-panel__head"><div><h2 class="dash-panel__title">Perlu Perhatian</h2><p class="dash-panel__subtitle">Notaris yang belum melengkapi seluruh bulan wajib pada periode terpilih.</p></div></div>
        <?php if ($dashboard_attention): ?>
          <div class="table-responsive dash-scroll-area">
            <table class="attention-table">
              <thead><tr><th>Notaris</th><th>Wilayah</th><th>Bulan Belum Dilaporkan</th></tr></thead>
              <tbody>
                <?php foreach ($dashboard_attention as $attention): ?>
                  <tr>
                    ...
                    <td><span class="attention-badge"><?= _ent($attention['missing_month_labels']); ?></span></td>
                  </tr>
                <?php endforeach; ?>
              </tbody>
            </table>
